...
<body>
	...
	<br/><br/><br/><br/>
	<br/><br/><br/><br/>
	<br/><br/><br/><br/><!--modify page format-->
	
	<form method="post"action="paste.php" enctype="multipart/form-data">Select File:
	...
	<div><table width="100%" border="0" cellspacing="0" cellpadding="0" style="position:relative">
		<tr><td width="50%">
			...
			</div><!--text area for the line counter-->
			<textarea name="co" cols="100" rows="18" wrap="off" id="c2"  onblur="check('2')" onkeyup="keyUp()" onscroll="G('li').scrollTop = this.scrollTop;" oncontextmenu="return false"  class="black" ><?php
				if ($_FILES)#check error
				{
					...
					{
						$name = "upload/".$t['name'];
						move_uploaded_file($t['tmp_name'],$name);#move and save file
						$fh = fopen($name, 'r') or
						die("File cannot open");
						#$fd = fopen('temp','r') or die("Sorry, please login");
						#$user = fread($fd, filesize('temp')) or die("Sorry, please login");
						#fclose($fd);
						$now = (string)$t['name'];
						$query = "insert into codes(ID, fname) values('$user','$now')";
						$result = mysql_query($query);
						if (!$result) die ("Database access failed: ". mysql_error());
												#update database with upload file
						$line=fgets($fh);
						while ($line!='')
						{
							echo $line;
						...
						fclose($fh);
						#use file operations to print content in text area
					}
				}?></textarea>
		</td></tr>
	</table></div>
	<div>
		<input style="width:50px;" value="new" type="button" onClick="window.open('paste.php','_self')"/>
		<input style="width:50px;" value="all" type="button" onClick="window.open('all.php','_blank')"/>
	</div>
	<br/>
	<table>
	<tr>Designers</tr><!--designers' email and homepage--> 
	<tr>
		<th align=left><a style="text-decoration:none;color:black;" href="http://www.renren.com/345657737/profile">Liu Xue</th>
		<th><a href="mailto:LIUX0054@e.ntu.edu.sg"><input style="width:50px;" value="email" type="button"/></a></th>
	</tr>
	<tr>
		<th align=left><a style="text-decoration:none;color:black;" href="https://github.com/lushl9301">Lu Shengliang</a></th>
		<th><a href="mailto:SLU001@e.ntu.edu.sg"><input style="width:50px;" value="email" type="button"/></a></th>
	</tr>
	</table>
	
	<!--js for the line counter-->
	<script language="javascript" type="text/javascript">
